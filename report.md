...
4. **Build Docker Image** – Finally, the pipeline calls `docker.build("calculator-app:${env.BUILD_NUMBER}")` to build a container from the repository’s `Dockerfile`.  The Jenkins documentation states that `docker.build()` creates a new image from the `Dockerfile` and returns an image object that can be used to push to a registry【713532891104427†L470-L516】.
